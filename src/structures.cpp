...
void Mesh::saveHDF5(const char *filename) {
	...
	if (!rank) {
		...
		H5Gcreate(file, "/ENS_MAA", 0);
		hid_t gMesh = H5Gcreate(file, "/ENS_MAA/mesh", 0);

		hid_t aid, attr;
		int DIM = 2, TYP = 0;
		char DES[] = " ";
		char UNV[] = "made by japet";
		aid = H5Screate(H5S_SCALAR);

		hid_t strType = H5Tcopy(H5T_C_S1);
		H5Tset_size(strType, 81);

		attr = H5Acreate(gMesh, "DIM", H5T_NATIVE_INT, aid, H5P_DEFAULT);
		H5Awrite(attr, H5T_NATIVE_INT, &DIM);
		H5Aclose(attr);
		attr = H5Acreate(gMesh, "TYP", H5T_NATIVE_INT, aid, H5P_DEFAULT);
		H5Awrite(attr, H5T_NATIVE_INT, &TYP);
		H5Aclose(attr);
		attr = H5Acreate(gMesh, "UNV", strType, aid, H5P_DEFAULT);
		H5Awrite(attr, strType, UNV);
		H5Aclose(attr);
		H5Tset_size(strType, 201);
		attr = H5Acreate(gMesh, "DES", strType, aid, H5P_DEFAULT);
		H5Awrite(attr, strType, DES);
		H5Aclose(attr);

		H5Gcreate(file, "/ENS_MAA/mesh/NOE", 0);
		int vAllSize = 0, vSize;
		int vSizes[size];
		vSize = (int) vetrices.size();
		MPI_Gather(&vSize, 1, MPI_INT, vSizes, 1, MPI_INT, 0, PETSC_COMM_WORLD);
		...
		dataset
				= H5Dcreate(file, "/ENS_MAA/mesh/NOE/COO", datatype, dataspace, H5P_DEFAULT);

		char NOM[] = "x               y        ";
		char UNI[] = "m               m";

		attr = H5Acreate(dataset, "NBR", H5T_NATIVE_INT, aid, H5P_DEFAULT);
		H5Awrite(attr, H5T_NATIVE_INT, &vAllSize);
		H5Aclose(attr);
		int REP = 0;
		attr = H5Acreate(dataset, "REP", H5T_NATIVE_INT, aid, H5P_DEFAULT);
		H5Awrite(attr, H5T_NATIVE_INT, &REP);
		H5Aclose(attr);
		H5Tset_size(strType, 33);
		attr = H5Acreate(dataset, "NOM", strType, aid, H5P_DEFAULT);
		H5Awrite(attr, strType, NOM);
		H5Aclose(attr);
		attr = H5Acreate(dataset, "UNI", strType, aid, H5P_DEFAULT);
		...
		hsize_t start[1], count[1], stride[1];
		start[0] = 0;
		count[0] = 2;
		stride[0] = vAllSize;
		...
		hsize_t memDim[1];
		memDim[0] = 2;
		hid_t memDS = H5Screate_simple(1, memDim, NULL);

		double temp[2];
		int *ids = new int[vAllSize];
		int *fam = new int[vAllSize];
		...
		for (std::map<PetscInt, Point*>::iterator i = vetrices.begin(); i
			...
			temp[0] = i->second->x;
			temp[1] = i->second->y;
			ids[start[0]] = i->first + 1;
			fam[start[0]] = i->second->family;
			H5Sselect_hyperslab(dataspace, H5S_SELECT_SET, start, stride, count, NULL);
			H5Dwrite(dataset, H5T_IEEE_F64LE, memDS, dataspace, H5P_DEFAULT, temp);
			start[0]++;
		}
		for (int i = 1; i < size; i++)
			for (int j = 0; j < vSizes[i]; j++) {
				...
				double coords[3];
				MPI_Recv(&id, 1, MPI_INT, i, 0, PETSC_COMM_WORLD, &stat);
				MPI_Recv(&family, 1, MPI_INT, i, 0, PETSC_COMM_WORLD, &stat);
				MPI_Recv(coords, 3, MPI_DOUBLE, i, 0, PETSC_COMM_WORLD, &stat);

				ids[start[0]] = id + 1;
				fam[start[0]] = family;
				H5Sselect_hyperslab(dataspace, H5S_SELECT_SET, start, stride, count, NULL);
				H5Dwrite(dataset, H5T_IEEE_F64LE, memDS, dataspace, H5P_DEFAULT, coords);
				start[0]++;
			}
		...
		dataset
				= H5Dcreate(file, "/ENS_MAA/mesh/NOE/NUM", datatype, dataspace, H5P_DEFAULT);
		attr = H5Acreate(dataset, "NBR", H5T_NATIVE_INT, aid, H5P_DEFAULT);
		H5Awrite(attr, H5T_NATIVE_INT, &vAllSize);
		H5Aclose(attr);
		H5Dwrite(dataset, H5T_NATIVE_INT32, H5S_ALL, H5S_ALL, H5P_DEFAULT, ids);
		H5Dclose(dataset);
		delete[] ids;

		dataset
				= H5Dcreate(file, "/ENS_MAA/mesh/NOE/FAM", datatype, dataspace, H5P_DEFAULT);
		attr = H5Acreate(dataset, "NBR", H5T_NATIVE_INT, aid, H5P_DEFAULT);
		H5Awrite(attr, H5T_NATIVE_INT, &vAllSize);
		H5Aclose(attr);
		H5Dwrite(dataset, H5T_NATIVE_INT32, H5S_ALL, H5S_ALL, H5P_DEFAULT, fam);
		H5Dclose(dataset);
		delete[] fam;
		...
		H5Gcreate(file, "/ENS_MAA/mesh/MAI", 0);
		H5Gcreate(file, "/ENS_MAA/mesh/MAI/TR3", 0);
		dim[0] = eAllSize * 3;

		dataspace = H5Screate_simple(1, dim, NULL);
		dataset
				= H5Dcreate(file, "/ENS_MAA/mesh/MAI/TR3/NOD", datatype, dataspace, H5P_DEFAULT);
		attr = H5Acreate(dataset, "NBR", H5T_NATIVE_INT, aid, H5P_DEFAULT);
		H5Awrite(attr, H5T_NATIVE_INT, &eAllSize);
		H5Aclose(attr);
		start[0] = 0;
		count[0] = 3;
		stride[0] = eAllSize;
		...
		memDim[0] = 3;
		memDS = H5Screate_simple(1, memDim, NULL);

		ids = new int[eAllSize];
		fam = new int[eAllSize];
		...
		for (std::map<PetscInt, Element*>::iterator i = elements.begin(); i
			...
			for (int k = 0; k < 3; k++)
				buf[k] = i->second->vetrices[k] + 1;
			H5Dwrite(dataset, H5T_NATIVE_INT, memDS, dataspace, H5P_DEFAULT, buf);
			start[0]++;
		}
		...
		dataspace = H5Screate_simple(1, dim, NULL);
		dataset
				= H5Dcreate(file, "/ENS_MAA/mesh/MAI/TR3/NUM", datatype, dataspace, H5P_DEFAULT);
		H5Dwrite(dataset, H5T_NATIVE_INT32, H5S_ALL, H5S_ALL, H5P_DEFAULT, ids);
		attr = H5Acreate(dataset, "NBR", H5T_NATIVE_INT, aid, H5P_DEFAULT);
		H5Awrite(attr, H5T_NATIVE_INT, &eAllSize);
		H5Aclose(attr);
		...
		dataset
				= H5Dcreate(file, "/ENS_MAA/mesh/MAI/TR3/FAM", datatype, dataspace, H5P_DEFAULT);
		H5Dwrite(dataset, H5T_NATIVE_INT32, H5S_ALL, H5S_ALL, H5P_DEFAULT, fam);
		attr = H5Acreate(dataset, "NBR", H5T_NATIVE_INT, aid, H5P_DEFAULT);
		H5Awrite(attr, H5T_NATIVE_INT, &eAllSize);
		H5Aclose(attr);
		...
		hid_t gInfo = H5Gcreate(file, "INFOS_GENERALES", 0);

		int MAJ = 2, MIN = 3, REL = 6;

		attr = H5Acreate(gInfo, "MAJ", H5T_NATIVE_INT, aid, H5P_DEFAULT);
		H5Awrite(attr, H5T_NATIVE_INT, &MAJ);
		H5Aclose(attr);
		attr = H5Acreate(gInfo, "MIN", H5T_NATIVE_INT, aid, H5P_DEFAULT);
		H5Awrite(attr, H5T_NATIVE_INT, &MIN);
		H5Aclose(attr);
		attr = H5Acreate(gInfo, "REL", H5T_NATIVE_INT, aid, H5P_DEFAULT);
		H5Awrite(attr, H5T_NATIVE_INT, &REL);
		H5Aclose(attr);
		...
		for (std::map<PetscInt, Point*>::iterator i = vetrices.begin(); i
				!= vetrices.end(); i++) {
			int id = i->first;
			int family = i->second->family;
			double coords[] = { i->second->x, i->second->y, i->second->z };

			MPI_Send(&id, 1, MPI_INT, 0, 0, PETSC_COMM_WORLD);
			MPI_Send(&family, 1, MPI_INT, 0, 0, PETSC_COMM_WORLD);
			MPI_Send(coords, 3, MPI_DOUBLE, 0, 0, PETSC_COMM_WORLD);
		}
