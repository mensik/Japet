...
void save2DResultHDF5(const char *filename, const char *name, Vec v) {
	...
	if (!rank) {
		...
		med_idt fid = MEDouvrir((char*) filename, MED_LECTURE_ECRITURE);
		MEDchampCr(fid, (char*) name, MED_FLOAT64, NOM, UNI, 2);
		...
		MEDchampEcr(fid, "mesh", (char*) name, (unsigned char *) out, MED_FULL_INTERLACE, globalSize/2, MED_NOGAUSS, 1, MED_NOPFL, MED_NO_PFLMOD, MED_NOEUD, MED_NONE, MED_NOPDT, "", 0.0, MED_NONOR);
		MEDchampEcr(fid, "mesh", (char*) name, (unsigned char *) out, MED_FULL_INTERLACE, globalSize/2, MED_NOGAUSS, 2, MED_NOPFL, MED_NO_PFLMOD, MED_NOEUD, MED_NONE, MED_NOPDT, "", 0.0, MED_NONOR);
	} else {
		MPI_Gather(&localSize, 1, MPI_INT, NULL, 1, MPI_INT, 0, PETSC_COMM_WORLD);
		MPI_Gatherv(locArray, localSize, MPI_DOUBLE, NULL, NULL, NULL, MPI_DOUBLE, 0, PETSC_COMM_WORLD);
	}
